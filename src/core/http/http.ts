...
async function SendApi(options: ApiSendConfig) {
  ...
  try {
    ...
    if (
      res.data.success === API_CONFIG.successCode ||
      (res.data as any).access_token ||
      (res.data as any).value ||
      (res.data as any).code === API_CONFIG.successCode
    ) {
      API_CONFIG.successHandle && (await API_CONFIG.successHandle())
      return Promise.resolve([
        res.data.data || res.data || API_CONFIG.successMessage,
        null
      ])
    } else {
      ...
      return Promise.resolve([res.data.data, res.data.message || errorText])
    }
  } catch (e: any) {
    if (e.__CANCEL__) {
      // 如果请求被cancel，不做任何处理
    ...
    return Promise.resolve([null, e.response && e.response.data.message])
  }
}
...
export async function Send(options: ApiSendConfig = {}) {
  if (options.mock) {
    return new Promise<any>(resolve => {
      setTimeout(() => {
        resolve([options.mock, options.mock])
      }, options.delay || 0)
    })
  } else {
    if (options.params) options.params = cloneDeep(options.params)
